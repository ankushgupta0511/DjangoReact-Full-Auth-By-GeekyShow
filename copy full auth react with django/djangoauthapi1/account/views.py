...
class UserRagistrationView(APIView):
    ...
    def post(self, request,format=None):
        ...
        # if serializer.is_valid():
            ...
            toekn = get_tokens_for_user(user)   # calling to genrate tokens
            
            return Response({'token':toekn,'msg':'Registration Successfully!!'},status=status.HTTP_201_CREATED)
...
class UserLoginView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if user is not None:
                toekn = get_tokens_for_user(user)   # calling to genrate tokens
                return Response({'toekn':toekn,'msg':'Login Success!!'},status=status.HTTP_200_OK)
            else:
